...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str(),"infind","zancudo","infind/GRUPO2/ESP145/conexion/cocina",0,true,serializa_JSON(estado).c_str())) {
      ...
      //"{\"online\":false}"
     client.publish("infind/GRUPO2/ESP145/conexion/cocina",serializa_JSON(estado).c_str(),true); // Publicamos por el topic correspondiente el estado de conexión
     
     client.subscribe("infind/GRUPO2/ESP145/led/cmd/cocina"); //Me suscribo al topic del estado del GPIO 2
     client.subscribe("infind/GRUPO2/ESP145/switch/cmd/cocina"); //Me suscribo al topic del estado del GPIO 16

     client.subscribe("infind/GRUPO2/ESP145/config/cmd/cocina"); //Me suscribo al topic del estado del led
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if(strcmp(topic,"infind/GRUPO2/ESP145/switch/cmd/cocina")==0)
  {
    ...
    else  // En caso de no haber error, se hace la secuencia propuesta:
    {
      ...
      client.publish("infind/GRUPO2/ESP145/switch/status/cocina",serializa_JSONSWITCH(valor_SWITCH).c_str()); //Publico que he recibido el dato del switch
    }
  }
  ...
  if(strcmp(topic,"infind/GRUPO2/ESP145/led/cmd/cocina")==0)
  {
    ...
    else // En caso de no haber error, se hace la secuencia propuesta:
    {
     ...
     client.publish("infind/GRUPO2/ESP145/led/status/cocina",serializa_JSONLED(valor_LED).c_str() ); //Publico que he recibido el dato del led
     subirled++;
  ...
  free(mensaje); // libero memoria
    ...
    //------RECOGIDA DATOS------
    if(strcmp(topic,"infind/GRUPO2/ESP145/config/cmd/cocina")==0) // Se estudia el estado del topic específico
    {
      StaticJsonDocument<512> root; // el tamaño tiene que ser adecuado para el mensaje
...
void setup() {
  ...
  // OTA -> Actualizamos al empezar
      client.subscribe("infind/GRUPO2/ESP145/FOTA/cocina"); //Me suscribo al topic de la actualización
      Serial.println( "--------------------------" );
    ...
    unsigned long now=millis();
      ...
      if (now-inicio>650) // Cuando pasen 0.65 seg tras la interrupción
      {
        ...
        if (estado_led2==1) // Si estaba encendido, apagamos
          ...
          valor_LED.LED=LED_dato;
          valor_LED.origen="pulsador"; 
          client.publish("infind/GRUPO2/ESP145/led/status/cocina",serializa_JSONLED(valor_LED).c_str()); //Publico que he recibido el dato del switch
        }
...
//Publicamos el estado de conexion con retain flag=true
  ...
  if (actualiza==true) // mantenemos pulsado: ACTUALIZAMOS! 
    {
      client.subscribe("infind/GRUPO2/ESP145/FOTA/cocina"); //Me suscribo al topic de la actualización cuando se mantenga más de 2 segundos el botón flash pulsado
      // OTA
      Serial.println( "--------------------------" );
      Serial.println( "Comprobando actualización:" );
      Serial.print(HTTP_OTA_ADDRESS);Serial.print(":");Serial.print(HTTP_OTA_PORT);Serial.println(HTTP_OTA_PATH); // Se comprueba la actualización
